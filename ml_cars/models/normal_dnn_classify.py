        ...
        if summaryout:
            ...
            try:
                with open('./{}_summary.log'.format(self.model.name), 'w') as s:
                    ...
                    if self.top is not None:
                        self.top.summary(print_ln=lambda x:t.write(x+NEWLINECODE))
...
class NormalDNNCrossValidation(__BaseModels):
    def __init__(self, X, y, names, gpusave=False, summary=False, summaryout=False,
            modelname='sample1', optname='adam', fc_nodes=1024, fc_act='relu',
            alpha=0.3, theta=1.0, optflag=True, custom=True, splitrate=0.2, **kwargs):
        super().__init__()
    ...
    #@Timer.timer
    def cv_train(self, epochs=100, batch_size=32, verbose=0, splitrate=0.2, K=5):
